...
function calculate() {
				...
				*/
				...
				var modeSelector = parseFloat(document.getElementById('mode').value);
				var s_braking = 0;
				var s_brakingaux = 0;
				var t_braking_s = 0;
				var t_brakingaux = 0;
				...
				if(v_own < 0 || v_ahead < 0 || s_range_m < 0 || a_ahead < -5.5 || isNaN(v_own) || isNaN(v_ahead) || isNaN(a_ahead) || isNaN(s_range_m)){
					...
					if(t_crash_s == -1){
						...
						if(a_ahead == 0){							
							...
							tmp_resultArray = calc_t_braking_noAcceleration(v_own, v_ahead);
							t_braking_s = tmp_resultArray[1];
							t_haptic_s = tmp_resultArray[0];
							
							//calculate the remaining waiting time
							t_nothing_s = calc_t_nothing_noAcceleration(t_haptic_s, s_range_m, v_own, v_ahead);
						}
						else{
							if(a_ahead<0){
								t_brakingaux = 	v_ahead/-a_ahead;
								s_brakingaux = s_range_m + v_ahead*t_brakingaux + 0.5*a_ahead*t_brakingaux*t_brakingaux;
								if(t_brakingaux<t_crash_s){
									//recalculate t_crash_s with a car ahead that has stopped
									t_crash_s = calcBrakeNecc(v_own, 0, 0, s_brakingaux);
									//calculating the braking time
									tmp_resultArray = calc_t_braking_noAcceleration(v_own, 0);
									t_braking_s = tmp_resultArray[1];
									t_haptic_s = tmp_resultArray[0];
									
									//calculate the remaining waiting time
									t_nothing_s = calc_t_nothing_noAcceleration(t_haptic_s, s_brakingaux, v_own, 0);
											
								}
								else{
								//calculate the remaining waiting time
								t_nothing_s = calc_t_nothing_Acceleration(t_consthaptic_s, s_range_m, v_own, v_ahead, a_ahead);
								
								//calculating the braking time
								t_haptic_s = t_consthaptic_s;
								tmp_resultArray = calc_t_braking_Acceleration (t_nothing_s, s_range_m, v_own, v_ahead, a_ahead);
								t_braking_s = tmp_resultArray[1];
								t_haptic_s = tmp_resultArray[0];
								
								if(getSpeed(v_own, (t_nothing_s + +t_warning_s + t_haptic_s), t_nothing_s, t_haptic_s, t_braking_s)<(v_ahead + a_ahead*(t_nothing_s + +t_warning_s + t_haptic_s))){
									//calculate the remaining waiting time
									t_nothing_s = calc_t_nothing_Acceleration_haptic(s_range_m, v_own, v_ahead, a_ahead);
									
									//calculating the braking time
									t_haptic_s = t_consthaptic_s;
									tmp_resultArray = calc_t_braking_Acceleration_haptic (t_nothing_s, s_range_m, v_own, v_ahead, a_ahead);
									t_braking_s = tmp_resultArray[1];
									t_haptic_s = tmp_resultArray[0];
									}
								}
						...
						//check whether to 0 or to relative speed
						
						if(modeSelector == 2 || a_ahead<0){
							if(t_braking_s = 0){
								var tempspeed = getSpeed(v_own, (t_nothing_s+t_warning_s+t_haptic_s), t_nothing_s, t_haptic_s, t_braking_s);
								t_haptic_s  = t_haptic_s + tempspeed/a_haptic_ms;
								if(t_haptic_s>t_consthaptic_s){
									t_haptic_s = t_consthaptic_s;
									t_braking_s = t_braking_s + (tempspeed-(a_haptic_ms*t_consthaptic_s))/a_eb_ms;
								}
							}
							else{
								var tempspeed = getSpeed(v_own, (t_nothing_s+t_warning_s+t_haptic_s+t_braking_s), t_nothing_s, t_haptic_s, t_braking_s);
								t_braking_s = t_braking_s + tempspeed/a_eb_ms;
							}
						}
...
function calc_t_braking_noAcceleration(v_own, v_ahead){
	
	var returnarray;
	var t_haptic_s;
	var t_braking_s 
	t_haptic_s = -(v_ahead - v_own)/a_haptic_ms;
	
	if(t_haptic_s>t_consthaptic_s){
		t_haptic_s = t_consthaptic_s
	}
	
	t_braking_s = -(v_ahead - v_own + a_haptic_ms*t_haptic_s)/a_eb_ms;
	
	returnarray = [t_haptic_s, t_braking_s];
	
	return returnarray;
}

function calc_t_nothing_noAcceleration(t_haptic_s, s_range_m, v_own, v_ahead){
	var t_nothing_s;
	
	t_nothing_s = -(s_range_m - s_puffer_m - v_own*(t_haptic_s + t_warning_s) + v_ahead*(t_haptic_s + t_warning_s - (v_ahead - v_own + a_haptic_ms*t_haptic_s)/a_eb_ms) + (v_ahead - v_own + a_haptic_ms*t_haptic_s) * (v_ahead - v_own + a_haptic_ms*t_haptic_s)/(2*a_eb_ms) + (a_haptic_ms*t_haptic_s * t_haptic_s)/2 + ((v_own - a_haptic_ms*t_haptic_s)*(v_ahead - v_own + a_haptic_ms*t_haptic_s))/a_eb_ms)/(v_ahead - v_own);
	
	return t_nothing_s;	
}

function calc_t_braking_Acceleration(t_nothing_s, s_range_m, v_own, v_ahead, a_ahead_ms){
	...
	t_braking_s = -(v_ahead - v_own + a_haptic_ms*t_haptic_s + a_ahead_ms*(t_haptic_s + t_nothing_s + t_warning_s))/(a_ahead_ms + a_eb_ms);
...
function calc_t_nothing_Acceleration(t_haptic_s, s_range_m, v_own, v_ahead, a_ahead_ms){
	
	var t_nothing_s, t_nothing_s1, t_nothing_s2;
	t_nothing_s1 =  (a_ahead_ms*(Math.sqrt((a_ahead_ms*a_haptic_ms*a_haptic_ms*t_haptic_s*t_haptic_s - a_ahead_ms*a_eb_ms*a_haptic_ms*t_haptic_s*t_haptic_s + a_eb_ms*v_ahead*v_ahead - 2*a_eb_ms*v_ahead*v_own + a_eb_ms*v_own*v_own + 2*a_ahead_ms*a_eb_ms*s_puffer_m - 2*a_ahead_ms*a_eb_ms*s_range_m)/(a_ahead_ms + a_eb_ms))) + a_eb_ms*(Math.sqrt((a_ahead_ms*a_haptic_ms*a_haptic_ms*t_haptic_s*t_haptic_s - a_ahead_ms*a_eb_ms*a_haptic_ms*t_haptic_s*t_haptic_s + a_eb_ms*v_ahead*v_ahead - 2*a_eb_ms*v_ahead*v_own + a_eb_ms*v_own*v_own + 2*a_ahead_ms*a_eb_ms*s_puffer_m - 2*a_ahead_ms*a_eb_ms*s_range_m)/(a_ahead_ms + a_eb_ms))) - a_eb_ms*v_ahead + a_eb_ms*v_own - a_ahead_ms*a_eb_ms*t_haptic_s + a_ahead_ms*a_haptic_ms*t_haptic_s - a_ahead_ms*a_eb_ms*t_warning_s)/(a_ahead_ms*a_eb_ms);
	t_nothing_s2 = -(a_ahead_ms*(Math.sqrt((a_ahead_ms*a_haptic_ms*a_haptic_ms*t_haptic_s*t_haptic_s - a_ahead_ms*a_eb_ms*a_haptic_ms*t_haptic_s*t_haptic_s + a_eb_ms*v_ahead*v_ahead - 2*a_eb_ms*v_ahead*v_own + a_eb_ms*v_own*v_own + 2*a_ahead_ms*a_eb_ms*s_puffer_m - 2*a_ahead_ms*a_eb_ms*s_range_m)/(a_ahead_ms + a_eb_ms))) + a_eb_ms*(Math.sqrt((a_ahead_ms*a_haptic_ms*a_haptic_ms*t_haptic_s*t_haptic_s - a_ahead_ms*a_eb_ms*a_haptic_ms*t_haptic_s*t_haptic_s + a_eb_ms*v_ahead*v_ahead - 2*a_eb_ms*v_ahead*v_own + a_eb_ms*v_own*v_own + 2*a_ahead_ms*a_eb_ms*s_puffer_m - 2*a_ahead_ms*a_eb_ms*s_range_m)/(a_ahead_ms + a_eb_ms))) + a_eb_ms*v_ahead - a_eb_ms*v_own + a_ahead_ms*a_eb_ms*t_haptic_s - a_ahead_ms*a_haptic_ms*t_haptic_s + a_ahead_ms*a_eb_ms*t_warning_s)/(a_ahead_ms*a_eb_ms);

	
	if(0>t_nothing_s1){
		t_nothing_s = t_nothing_s2;
	}
	else if(0>t_nothing_s2){
		t_nothing_s = t_nothing_s1;
	}
	else if(0<=t_nothing_s1 && 0<=t_nothing_s2){
		t_nothing_s = Math.min(t_nothing_s1, t_nothing_s2);
	}
	else{
		t_nothing_s = 0;
	}
...
function calc_t_braking_Acceleration_haptic(t_nothing_s, s_range_m, v_own, v_ahead, a_ahead_ms){
	
	var returnarray;
	var t_haptic_s;
	var t_braking_s;
	
	t_haptic_s = -(v_ahead - v_own + a_ahead_ms*(t_nothing_s + t_warning_s))/(a_ahead_ms + a_haptic_ms);
...
function calc_t_nothing_Acceleration_haptic(s_range_m, v_own, v_ahead, a_ahead_ms){
	
	var t_nothing_s, t_nothing_s1, t_nothing_s2;
	t_nothing_s1 = 	(a_ahead_ms*(Math.sqrt((a_haptic_ms*(v_ahead*v_ahead - 2*v_ahead*v_own + v_own*v_own + 2*a_ahead_ms*s_puffer_m - 2*a_ahead_ms*s_range_m))/(a_ahead_ms + a_haptic_ms))) + a_haptic_ms*(Math.sqrt((a_haptic_ms*(v_ahead*v_ahead - 2*v_ahead*v_own + v_own*v_own + 2*a_ahead_ms*s_puffer_m - 2*a_ahead_ms*s_range_m))/(a_ahead_ms + a_haptic_ms))) - a_haptic_ms*v_ahead + a_haptic_ms*v_own - a_ahead_ms*a_haptic_ms*t_warning_s)/(a_ahead_ms*a_haptic_ms);
	t_nothing_s2 = -(a_ahead_ms*(Math.sqrt((a_haptic_ms*(v_ahead*v_ahead - 2*v_ahead*v_own + v_own*v_own + 2*a_ahead_ms*s_puffer_m - 2*a_ahead_ms*s_range_m))/(a_ahead_ms + a_haptic_ms))) + a_haptic_ms*(Math.sqrt((a_haptic_ms*(v_ahead*v_ahead - 2*v_ahead*v_own + v_own*v_own + 2*a_ahead_ms*s_puffer_m - 2*a_ahead_ms*s_range_m))/(a_ahead_ms + a_haptic_ms))) + a_haptic_ms*v_ahead - a_haptic_ms*v_own + a_ahead_ms*a_haptic_ms*t_warning_s)/(a_ahead_ms*a_haptic_ms);
...
*/
	...
	if(t_now_s<=t_brakingaux || t_brakingaux == 0){
		...
		if(t_now_s < t_nothing_s + t_warning_s){
			...
			distance_s = distance_s-((v_own*(t_nothing_s+t_warning_s+t_haptic_s) - 0.5 * a_haptic_ms*(t_haptic_s)*(t_haptic_s))+(v_own - a_haptic_ms * t_haptic_s)*(t_braking_s) - 0.5 * a_eb_ms*(t_braking_s)*(t_braking_s) + (v_own - a_haptic_ms * t_haptic_s - a_eb_ms * t_braking_s) * (t_now_s-(t_nothing_s + t_warning_s + t_haptic_s+t_braking_s)));
		}
...
*/
	...
	if(t_braking_s != 0){
		...
		temp_dist_m = getDist(s_range_m, v_own, v_ahead, a_ahead, (t_nothing_s + +t_warning_s + t_haptic_s), t_nothing_s, t_haptic_s, t_braking_s, t_brakingaux, s_brakingaux);
		temp_speed_ms = getSpeed(v_own, (t_nothing_s + +t_warning_s + t_haptic_s), t_nothing_s, t_haptic_s, t_braking_s);
		document.getElementById('emergency_time').innerHTML = 'Vehicle already adapted speed after '+ Math.round((t_nothing_s + +t_warning_s + t_haptic_s)*100)/100 +' s';
		document.getElementById('emergency_dist').innerHTML = Math.round(temp_dist_m*100)/100+" m";
		document.getElementById('emergency_speed').innerHTML = Math.round(temp_speed_ms*100)/100+" m/s";
	}
	...
	for (var i = 0; i <= t_steps; i+=(t_steps/solution)){
		...
		if(i < t_nothing_s + t_warning_s){
			...
			dist_own = (v_own*(t_nothing_s+t_warning_s+t_haptic_s) - 0.5 * a_haptic_ms*(t_haptic_s)*(t_haptic_s))+(v_own - a_haptic_ms * t_haptic_s)*(t_braking_s) - 0.5 * a_eb_ms*(t_braking_s)*(t_braking_s) + (v_own - a_haptic_ms * t_haptic_s - a_eb_ms * t_braking_s) * (i-t_ges);
		}
	...
	if(checkvar){
		...
		temp_speed_ms = getSpeed(v_own, (t_nothing_s + +t_warning_s + t_haptic_s + t_braking_s), t_nothing_s, t_haptic_s, t_braking_s);
		...
		document.getElementById('stop_speed').innerHTML = Math.round(temp_speed_ms*100)/100+" m/s";
	}
	else{
		var divCollision = document.getElementById("collision");
		divCollision.style.display = "table-row";
