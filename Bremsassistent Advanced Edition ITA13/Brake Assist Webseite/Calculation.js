					...
					if(t_crash_s == -1){
						...
						if(a_ahead == 0){							
							...
							if(a_ahead<0){
								...
								//calculate the remaining waiting time
								t_nothing_s = calc_t_nothing_Acceleration(s_range_m, v_own, v_ahead, a_ahead);
...
function calc_t_braking_Acceleration(t_nothing_s, s_range_m, v_own, v_ahead, a_ahead_ms){
	...
	var t_haptic_s;
	var t_braking_s 
	t_haptic_s = -(v_ahead - v_own + a_ahead_ms*(t_nothing_s + t_warning_s))/(a_ahead_ms + a_haptic_ms);//(Math.sqrt(a_haptic_ms*(a_ahead_ms + a_haptic_ms)*(v_ahead*v_ahead - 2*v_ahead*v_own + v_own*v_own + 2*a_ahead_ms*s_puffer_m - 2*a_ahead_ms*s_range_m)))/(a_haptic_ms*a_haptic_ms + a_ahead_ms*a_haptic_ms);
	
	/*if(t_haptic_s>t_consthaptic_s){
		t_haptic_s = t_consthaptic_s;
	}*/
	
	t_braking_s = 0//-(v_ahead - v_own + a_haptic_ms*t_haptic_s + a_ahead_ms*(t_haptic_s + t_nothing_s + t_warning_s))/(a_ahead_ms + a_eb_ms);
...
function calc_t_nothing_Acceleration(s_range_m, v_own, v_ahead, a_ahead_ms){
	
	var t_nothing_s;
	t_nothing_s =  (a_ahead_ms*(Math.sqrt((a_haptic_ms*(v_ahead*v_ahead - 2*v_ahead*v_own + v_own*v_own + 2*a_ahead_ms*s_puffer_m - 2*a_ahead_ms*s_range_m))/(a_ahead_ms + a_haptic_ms))) + a_haptic_ms*(Math.sqrt((a_haptic_ms*(v_ahead*v_ahead - 2*v_ahead*v_own + v_own*v_own + 2*a_ahead_ms*s_puffer_m - 2*a_ahead_ms*s_range_m))/(a_ahead_ms + a_haptic_ms))) + a_haptic_ms*v_ahead - a_haptic_ms*v_own + a_ahead_ms*a_haptic_ms*t_warning_s)/(a_ahead_ms*a_haptic_ms);//(a_ahead_ms*(Math.sqrt((a_ahead_ms*a_haptic_ms*a_haptic_ms*t_haptic_s*t_haptic_s - a_ahead_ms*a_eb_ms*a_haptic_ms*t_haptic_s*t_haptic_s + a_eb_ms*v_ahead*v_ahead - 2*a_eb_ms*v_ahead*v_own + a_eb_ms*v_own*v_own + 2*a_ahead_ms*a_eb_ms*s_puffer_m - 2*a_ahead_ms*a_eb_ms*s_range_m)/(a_ahead_ms + a_eb_ms))) + a_eb_ms*(Math.sqrt((a_ahead_ms*a_haptic_ms*a_haptic_ms*t_haptic_s*t_haptic_s - a_ahead_ms*a_eb_ms*a_haptic_ms*t_haptic_s*t_haptic_s + a_eb_ms*v_ahead^2 - 2*a_eb_ms*v_ahead*v_own + a_eb_ms*v_own*v_own + 2*a_ahead_ms*a_eb_ms*s_puffer_m - 2*a_ahead_ms*a_eb_ms*s_range_m)/(a_ahead_ms + a_eb_ms))) - a_eb_ms*v_ahead + a_eb_ms*v_own - a_ahead_ms*a_eb_ms*t_haptic_s + a_ahead_ms*a_haptic_ms*t_haptic_s - a_ahead_ms*a_eb_ms*t_warning_s)/(a_ahead_ms*a_eb_ms);
